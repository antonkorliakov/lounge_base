...
function enteredFieldValue(field: Field, position: number): unknown {
  switch (field.type) {
    ...
    // Контактные поля проходят валидацию только в своём формате, так что
    // «различимо по ключу поля» — как у text/textarea выше — здесь
    // недостижимо буквально. Email всё же несёт ключ в локальной части
    // (уехавшая в чужую колонку ячейка не совпадёт со «своим» значением
    // случайно); телефон различим не по ключу, а по `position` — вторая
    // тройка цифр в номере своя у каждого поля этого типа, и этого хватает
    // для той же проверки: `100 + position` уже даёт три цифры без
    // дополнительного паддинга (полей меньше 900).
    case 'phone':
...
describe('обратный прогон: слой xlsx', () => {
  ...
  it('каждая из 510 ячеек читается обратно тем же значением того же типа', () => {
...
describe('обратный прогон: введённое возвращается (ожидания — от сида)', () => {
  ...
  /**
   * Профили сквозь выгрузку: колонок по-прежнему 510 — по семь на каждую
