...
export async function missingItems(
  ...
  // если позиция ЕСТЬ (available offered), одной этой отметки хватает лишь
  // позициям с профилем `none`; остальным нужны ещё атрибуты второго
  // прохода, которые требует их профиль (`requiredAttributesFor` в
  // `form-schema/services.ts`: chargeType, цена/валюта при платности,
  // details у позиций с подсказкой). Раньше chargeType требовался прямо в
  // validateServiceValue, из-за чего первый проход никогда не сохранялся
  // (R1, второй круг сквозного ревью); теперь это требование живёт только
  // тут, в полноте, а не в форме значения. `serviceItemAnswered` — общая
  // точка правды с `validateServiceValue` и с контрактным тестом.
