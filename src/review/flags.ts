 ...
 * заполняющий отредактировал ответ, ревьюер должен посмотреть блок заново, а
 * остальные блоки остаются подтверждёнными. Возвращает `true`, если замечание
 * было (то есть было что снимать).
 *
 * **Подтверждение блока снимается ВСЕГДА, а не только когда нашлось открытое
 * замечание.** Раньше `DELETE` стоял за `if (cleared.length === 0) return
 * false`, и это был первый Important целого обзора ветки: анкета в
 * `changes_requested`, последнее замечание которой уже снято предыдущей
 * правкой, проваливается сквозь ОБА гейта `FillForm` (экран правок требует
 * `flags.length > 0`) и показывает заполняющему всю форму из 19 шагов. Правка
 * любого НЕотмеченного ответа там проходила `assertEditable`, не находила
 * замечания — и оставляла блок этого ответа подтверждённым, после чего
 * `approveSubmission` видел 27/27 и переносил классифицирующие поля в
 * `lounges`. Возвращаемое значение по-прежнему отвечает ровно на свой вопрос
 * («было ли что снимать» — от него зависит только логирование у вызывающих),
 * но оно больше не решает, инвалидировать ли подтверждение: подтверждение
 * относится к ДАННЫМ, а не к замечанию.
 *
 * Это не дублирует вывод `blockProgress` (`blocks.ts`), который считает блок
 * неподтверждённым, когда `confirmedAt` старше времени записи ответов, — это
 * его дополнение, и нужно оно ровно для того, чего штамп времени увидеть не
 * может: УДАЛЕНИЕ не оставляет ни строки, ни времени. Убранный снимок делает
 * самый свежий `photos.uploadedAt` слота не новее, а СТАРШЕ, так что для
 * `DELETE /api/photos` производный признак слеп, а этот вызов — нет (маршрут
 * зовёт `clearFlagAfterSave` и на удалении тоже). Второй, более узкий случай:
 * `Date` в JS обрезает микросекунды постгреса до миллисекунд, поэтому правка,
 * попавшая в ту же миллисекунду, что и подтверждение, для `blockProgress`
 * неотличима от «была раньше» — здесь она снимает подтверждение независимо от
 * разрешения часов.
 *
 * Цена — один лишний `DELETE` (обычно ни во что не попадающий) на каждое
 * автосохранение: он выполняется в уже открытой транзакции с уже взятой
 * блокировкой, по тому же уникальному ключу `(submission_id, block_key)`.
 *
