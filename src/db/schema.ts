 ...
 * что команда когда-либо касалась ответа (события `answer_edited_by_team` в
 * `events` пишутся с парой old→new под ту историю, но ЧИТАТЕЛЯ у них пока
 * нет — ни экрана, ни выгрузки; история здесь — записанная возможность, а не
