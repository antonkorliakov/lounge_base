    ...
    <FormShell status={statusText} onSubmit={submit}>
      {(step) => {
        ...
        if (step.kind === 'services1') {
          // Значок провенанса — И на строке первого прохода: позиция,
          // которую команда ЗАКРЫЛА («no»/«not_allowed»), выпадает из
          // offered-фильтра второго прохода, и первый — единственное место
          // основной формы, где этот ответ вообще виден. Прежний комментарий
          // здесь утверждал, что закрытая командой позиция видна «на карточке
          // правок, если её отметили», — ложная посылка: правка команды
          // снимает замечание своего ключа, и неотмеченная позиция карточки
          // не имела вовсе (аудит, must-fix 2).
          return (
            <ServicesPass1
              values={services}
        ...
        if (step.kind === 'services2') {
          // Значок и на карточке деталей: у ПРЕДЛОЖЕННОЙ позиции ответ
          // команды виден именно здесь.
          return (
            <ServicesPass2
              values={services}
              onChange={changeService}
