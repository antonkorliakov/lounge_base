 ...
 * — в подвале), и отклик появляется там, откуда действие вызвали, — рядом с
 * нажатой кнопкой, а не в другом углу экрана. С тех пор как экран стал рамой
 * (шапка — всегда видимый первый ряд, см. .review-screen в globals.css), оба
 * места на экране постоянно, но правило не меняется: отклик flag/unflag идёт
 * в подвал, к паре по блоку, — замечание ставится в строке блока, и подвал ей
 * и семантически ближе (решение по этому же блоку), и геометрически.
 */
...
export function ReviewScreen(props: {
  ...
  return (
    <div className="review-screen">
      {/* Чья анкета и в каком она состоянии — до всего остального И на всю
          ширину окна: шапка — первый ряд сетки, на обе колонки (см.
          .review-screen в globals.css), а не жилец правой панели. Раньше её
          поджимала рейка навигации, хотя ни дорога назад, ни статус, ни
          решения по всей анкете к открытому блоку не относятся. Экран
          показывал 27 блоков решений, не называя ни лаунжа, ни состояния:
        ...
        {keys.map((key) => {
          ...
          return (
            <FieldRow
              ...
              // списка, и прилепленный подвал — ближайшее к строке место из
              // двух зон отклика (см. `FeedbackScope`).
