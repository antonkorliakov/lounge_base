 ...
 * в структуре нет.
 *
 * Ночные интервалы через полночь оператор вводит ОДНИМ диапазоном в редакторе
 * правил («22:00–02:00» — одна пара полей, один диапазон), а хранится он уже
 * разложенным по суткам: `expandRules` делит его на «22:00–24:00» сегодня и
 * «00:00–02:00» завтра (см. `isNightRange`/`expandRules`). Поэтому конец
 * ХРАНИМОГО интервала всегда больше начала, и сравнение границ — обычное
 * сравнение минут от полуночи, без ветки «а вдруг это следующий день»; ветка
 * «через полночь» живёт ровно один раз, в `expandRules`, а не размазана по
 * тем, кто читает уже разложенную неделю.
 */
...
/** Конец суток, хранится как «24:00». Редактор держит ОДНО представление
 *  этого значения — «00:00» в поле «до» (правило I1: `rangeToWindow`/
 *  `windowToRange` — единственный перевод туда и обратно), поэтому
 *  `END_OF_DAY` попадает в хранение из двух источников: `expandRules`,
 *  разбивающий ночной диапазон («22:00–02:00») по суткам, и оператор,
 *  набравший «00:00» или «24:00» как конец — оба написания `parseClockInput`
 *  переводит в то же «00:00» ещё до того, как оно попадёт в `Range`.
 *  Сегодняшняя половина ночного диапазона закрывается им, а не 23:59, иначе
 *  «работаем до полуночи» теряло бы минуту. Здесь это законное значение `to`,
 *  которое сравнивают `clockMinutes` наравне с обычным временем. */
...
export type WindowsProblem = 'shape' | 'clock' | 'order' | 'overlap' | 'empty' | 'markerBeside' | null
...
export function windowsProblem(value: unknown): WindowsProblem {
  ...
  for (const window of value) {
    ...
    // в дне. Второй рядом с ним (до или после) — свой тег, не общий `order`
    // (I3, сквозное ревью): у оператора нет способа «упорядочить» маркер
    // относительно часов, и почти всегда это не его ошибка, а ночной хвост
    // предыдущего дня, наехавший на маркерный день (`collapseWeek` не может
    // склеить хвост с маркером — маркеру некуда принять чужое время). Общий
    // `order` не сказал бы оператору, что тут вообще делать.
    const marker = from === FIRST_FLIGHT || to === LAST_FLIGHT
    if (marker && value.length > 1) return 'markerBeside'
 ...
 * `'flightNotAllowed'`, `'markerBeside'` — да: день по-прежнему
 * `{ kind, windows: Window[] }` пусть и с неверными временами, недопустимым
 * `allDay`/маркером или маркером рядом с чужим окном, редактор читает его как
 * обычно и просто рисует то, что там есть, а отказ на сохранение (если он
 * будет) покажет `validateField`.
 *
 ...
 * `'kind'`, `'allDayNotAllowed'`, `'flightNotAllowed'`, `'clock'`, `'order'`,
 * `'overlap'`, `'markerBeside'` — они приходят с ОДНОГО испорченного дня
 * еженедельной уборки (`weekHoursProblem`
...
/** I3 (сквозное ревью): текст отказа для `windowsProblem`'s `'markerBeside'` —
 *  живёт рядом со схемой, а не только в `validation.ts`, потому что
 *  `HoursRulesEditor` тоже показывает его как клиентский намёк (сервер
 *  остаётся воротами; клиент — подсказка, чтобы отказ не пришёл издалека
 *  после нажатия «отправить»). Называет ОБЕ вещи: что нельзя (маркер рядом с
 *  другим окном) и что обычно к этому приводит (ночной хвост предыдущего
 *  дня) — иначе оператор видел бы правильный с виду день и не понимал, что
 *  чинить. */
export const MARKER_BESIDE_TEXT: Localized = {
  en: '“First flight” / “last flight” cannot share a day with other hours — here a night schedule from the previous day runs into it. Give this day clock times, or end the previous day at 24:00.',
  ru: '«Первый рейс» / «последний рейс» не сочетаются с другими часами в тот же день — сюда переходит ночной график предыдущего дня. Укажите этому дню время или закончите предыдущий день в 24:00.',
}
 ...
 *
 *  «До 00:00» (часовое ИЛИ маркерное начало, см. `rangeToWindow`) печатается
 *  как `END_OF_DAY` («24:00») — «00:00» в `Range` (единственное представление
 *  конца суток, которое держит редактор, I1) не имеет права утечь читателю
 *  как есть: сохранённая неделя и так хранит настоящее «24:00»
 *  (`expandRules`), и текст обязан читаться одинаково независимо от того,
 *  пришло ли значение из формы или прямо из базы. `rangeToWindow` — ЕДИНСТВЕННОЕ
 *  место, которое решает этот вопрос (I1, сквозное ревью); раньше эта же
 *  проверка была переписана здесь заново и не знала про маркерное начало. Без
 *  пометки «(next day)» — это не ночь, `isNightRange` уже вернула бы `false`
 *  для такого диапазона. */
export function formatRange(range: Range, locale: 'en' | 'ru'): string {
  if (range.from === '') return UNANSWERED
  const text = formatWindow(rangeToWindow(range), locale)
 ...
 *  (`END_OF_DAY` в единственном представлении, которое держит `Range`, I1 —
 *  буквального «24:00» там не бывает: `parseClockInput` переводит и набранное
 *  «00:00», и набранное «24:00» в одно и то же значение до того, как оно
 *  попадёт в `Range`), а не начало следующего дня. Ночной хвост,
 ...
 * суток, поэтому в хранении стоит `24:00` (значение, которого сам `Range` не
 * хранит — оператор набирает «00:00», единственное представление конца
 * суток, которое держит редактор, I1). Одно правило для обоих писателей:
 * правил часов (`expandRules`) и уборки
 ...
 * день ещё не отвечен, см. `emptyRule`/`rulesFromWeek`, где `to` — `null`, не
 * «00:00»): отвечать там ещё нечему, а «00:00» в `to` при пустом `from` может
 * остаться только от предыдущего ввода (оператор снял начало кнопкой «×» у
 * маркера, поле «до» не тронул) — это не осмысленный ответ «до конца суток»
 * без своего начала. */
 ...
 *  представление конца суток, которое держит `Range` (I1); хранимый
...
export function collapseWeek(week: WeekHours): HoursRule[] {
  ...
  // }` через `windowToRange` — единственным представлением конца суток,
  // которое держит `Range` (I1) (Critical, Task 5; сама функция —
