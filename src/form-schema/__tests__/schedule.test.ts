...
describe('канонический текст недельных часов', () => {
  ...
  // C2 (whole-branch fix wave): `collapseWeek` впитывает ночной хвост дня в
  // ПРЕДЫДУЩИЙ день как один ночной диапазон — сам день после этого ни в
  // одно правило не попадает (`rules.find` не находит его), и старый код
  // падал в ветку «неотвечено» прочерком, хотя `weekHoursCells` печатал бы
  // его часы честно («00:00–01:00»). Четверговая ночь легитимно даёт пятнице
  // именно такой хвост без своего собственного правила.
  it('C2: день, чьи единственные часы — ночной хвост предыдущего дня, печатает эти часы, а не прочерк', () => {
    const week: WeekHours = {
      mon: { kind: 'none' }, tue: { kind: 'none' }, wed: { kind: 'none' },
      thu: { kind: 'windows', windows: [{ from: '22:00', to: END_OF_DAY }] },
      fri: { kind: 'windows', windows: [{ from: '00:00', to: '01:00' }] },
      sat: { kind: 'none' }, sun: { kind: 'none' },
    }
    const texts = dayTexts(week, OPEN, 'en')
    // Четверг сохраняет своё правило как обычно (склейка ночи, не задета
    // фиксом) — часы читаются от 22:00 до 01:00 хвоста пятницы.
    expect(texts.thu).toBe('22:00–01:00 (next day)')
    // Пятница — сам фикс: её единственные часы («00:00–01:00») ушли в
    // правило четверга и не образуют собственного правила; раньше это
    // читалось прочерком.
    expect(texts.fri).toBe('00:00–01:00')
    expect(formatWeekHours(week, OPEN, 'en')).toContain('Fri 00:00–01:00')
  })
})
...
// Critical review finding (Task 5): в `Range` конец суток хранится как
// «00:00», не буквальный «24:00» — оператор может набрать и «24:00», но
// `parseClockInput` переводит его в то же «00:00» ещё до того, как оно
// попадёт в `Range` (I1). Правило (принято): `to: '00:00'` при часовом
// `from` значит «конец ЭТИХ суток», а не ночь через полночь — это касается
// и полного дня `00:00–00:00` (полночь-в-полночь).
...
describe('collapseWeek — дни собираются в правила', () => {
  ...
  it('полный день 00:00–24:00 не считается хвостом предыдущего', () => {
    ...
    // (Critical, Task 5) — единственное представление конца суток, которое
    // держит `Range` (I1); у обоих дней здесь его и нет.
  ...
  // `ClockInput` показывает то, что лежит в `Range`, а буквального `'24:00'`
  // там не бывает (I1), так что поле «до» осталось бы пустым, хотя итог под
  // редактором и так печатает «24:00».
...
describe('canAddRange — второй интервал правила (C1)', () => {
  ...
  // Critical review finding (Task 5): `to: '00:00'` — единственное
  // представление «до конца суток», которое держит `Range` (I1) — должно
  // занимать день так же, как раньше это делал буквальный `'24:00'`.
