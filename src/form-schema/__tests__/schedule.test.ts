...
describe('nextWindowStart — с какого времени предложить следующий интервал', () => {
  ...
  it('последний интервал недописан — начинаем от его собственного начала, оператор его ещё передвинет', () => {
    expect(nextWindowStart([{ from: '09:00', to: null }])).toBe('09:00')
  })
  ...
  // фикстурах выше с непустым результатом — вместо того чтобы гадать ещё
  // случаи, дописываем ровно то, что реально предлагает кнопка.
  //
  // Список НЕ включает `[{from:'09:00',to:null}]`: у недописанного последнего
  // интервала `nextWindowStart` намеренно возвращает его же `from` (см. тест
  // выше), а `windowsProblem` запрещает двум интервалам начинаться в одну
  // минуту — `start <= previousStart` тем же значением всегда даёт 'order',
  // какой бы `from` мы ни предложили. Это не повод менять `windowsProblem`
  // (правило верное — незачем и второму интервалу дня стартовать в ту же
  // минуту, что первому) и не повод менять пинованное значение выше (оно —
  // лучшее, что можно предложить, не зная, где оператор остановит первый
  // интервал). Для ЭТОГО фикстура свойство не проверяем: оно здесь физически
  // не выполнимо, а не нарушено багом.
  describe('свойство: то, что предлагает кнопка, всегда достраивает годный список', () => {
    ...
    it.each(fixtures.map((windows) => [windows] as const))('%j + предложенный интервал остаётся годным списком', (windows) => {
