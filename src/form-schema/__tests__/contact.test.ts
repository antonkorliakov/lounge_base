...
describe('isValidPhone — серверное правило', () => {
  it.each(ACCEPTED_PHONES)('принимает %s', (text) => {
  ...
  it.each(ACCEPTED_PHONES)(
    'то, что фильтр набора пропускает без изменений, ворота тоже принимают: %s',
    (text) => {
      // `PHONE_CHAR` (фильтр набора) и разделители, которые здесь вырезает
      // `.replace(/[\s()-]/g, '')` (ворота), — два независимых литерала,
      // которые обязаны соглашаться о том, что такое «разделитель» в
      // телефоне. Разойдись они — например, вычеркни кто-то `-` из строки
      // ворот — фильтр продолжил бы пропускать дефис в поле, а сервер начал
      // бы отказывать то же самое значение: набранное вводом становится
      // несохраняемым. Этот тест — тот, что падает первым при таком дрейфе.
      expect(isValidPhone(sanitizePhoneInput(text))).toBe(true)
    },
  )
})
