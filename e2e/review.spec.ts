...
test('вход в кабинет: ответ формы не выдаёт состав команды, ссылка одноразовая', async ({
  ...
  // кабинет. Это и есть проверка `requestLoginAction`'s счастливого пути: до
  // сих пор у него не было ни одного теста, и исправление с `after()` (письмо
  // отправляется после ответа, но обязано отправиться) держалось на одном
  // ручном наблюдении.
