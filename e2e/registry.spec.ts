...
/**
 * Сколько ждать, пока реестр ОЖИВЁТ (гидрируется), в хелперах ниже.
 *
 * Не «на всякий случай»: под пятью воркерами против одного `next dev` этот
 * файл падал на `toHaveURL` с прежним бюджетом 20 с, причём повтор всё это
 * время исправно щёлкал контролом — то есть клиент не был жив ДОЛЬШЕ
 * двадцати секунд (турбопак компилирует клиентский бандл `/admin` по
 * требованию, и пять воркеров просят его разом). 45 с укладываются в
 * тестовый бюджет (90 с, см. `playwright.config.ts`) и стоят времени только
 * на настоящем провале: успех выходит из цикла на первой итерации после
 * гидрации.
 */
const SETTLE = 45_000
 ...
 *
 * Повтор — та же гонка гидрации, что описана у `fillEnabling` в
 * `e2e/review.spec.ts`, и здесь она ДОКАЗАННАЯ: в полном параллельном прогоне
 * (пять файлов, пять воркеров) этот хелпер падал на `toHaveURL` в трёх
 * сценариях сразу и в разных сочетаниях от прогона к прогону —
 * воспроизводится и без `review.spec.ts` в наборе, то есть дело в нагрузке, а
 * не в соседе. `fill()` и `press('Enter')` ждут видимости, а не
 * интерактивности: до гидрации submit-обработчик формы поиска не навешен,
 * Enter не делает НИЧЕГО (обычной отправки формы тоже нет — переход собирает
 * клиентский роутер), и адрес не меняется никогда.
 *
 ...
 * повтор тем же текстом не даёт перехода значения. Успешный переход выходит из
 * цикла на первой же итерации после гидрации, поэтому повтор ничего не
 * скрывает: сломанный поиск исчерпает попытки и упадёт на том же `toHaveURL`.
 *
 * Бюджет — 45 с, а не 20 (см. `SETTLE`): под пятью воркерами `next dev`
 * компилирует клиентский бандл `/admin` по требованию, и гидрация тут
 * измеренно занимала БОЛЬШЕ двадцати секунд — повтор с прежним бюджетом
 * исчерпывался, так и не дождавшись живого клиента.
 */
...
/**
 * Выбирает значение в выпадающем фильтре и ждёт, пока выбор доедет до
 * адресной строки. Повтор — по той же причине, что у `searchFor` выше: до
 * гидрации `selectOption` меняет value в DOM, но `change` некому услышать, и
 * перехода не случается.
 *
 * В отличие от `searchFor`, сбрасывать контрол перед повтором НЕ НУЖНО, и это
 * измерено, а не предположено: у `<select>` React зовёт `onChange` на каждом
 * событии `change`, даже когда значение то же самое (проверено зондом —
 * повторный `selectOption('arrival')` на уже выбранном «arrival» дал новый
 * RSC-запрос, то есть переход состоялся). Value tracker, из-за которого
 * повтор без очистки бесполезен у текстового поля (см. `fillEnabling` в
 * `e2e/review.spec.ts`), к `<select>` не применяется.
 *
 * Сброс здесь был бы ХУЖЕ, чем ненужен: у этого контрола каждая смена — это
 * переход (`RegistryFilters` собирает новый URL), так что сброс сам уводит
 * страницу, а следующий выбор гонится с этим переходом и теряется. Проверено
 * живьём: вариант со сбросом крутился все 20 секунд и падал, оставаясь на
 * `/admin`.
 */
async function selectFilter(page: Page, label: string, value: string, url: RegExp): Promise<void> {
  const select = filterBar(page).getByLabel(label)
  await expect(async () => {
...
test('фильтр по аэропорту живёт в адресной строке и переживает перезагрузку', async ({
  ...
  // `filterOptions` отдаёт строки из базы как есть). Через `selectFilter`:
  // это ПЕРВОЕ взаимодействие с только что загруженной страницей, и до
  // гидрации `change` некому услышать (см. хелпер). Двум другим выборам зоны
  // в этом файле повтор не нужен: там перед ними уже прошёл `searchFor`, а
  // сменившийся адрес — доказательство живого клиента.
  await selectFilter(page, 'Airport', 'Dubai International', /airport=Dubai\+International/)
