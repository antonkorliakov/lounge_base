...
- Produces:
  ...
  - `openFlags(db, submissionId): Promise<FlagRow[]>`
...
export async function openFlags(db: Db, submissionId: string): Promise<FlagRow[]> {
  const rows = await db
    .select({
      id: fieldFlags.id,
...
**Interfaces:**
- Consumes: `Db`, `BLOCKS`, `FIELDS`, `SERVICE_GROUPS`, `SERVICE_ITEMS`, `openFlags`, таблица `blockReviews`
- Produces:
  - `confirmBlock(db, { submissionId, blockKey, reviewer }): Promise<ConfirmResult>`
  - `unconfirmBlock(db, { submissionId, blockKey }): Promise<void>`
  - `blockProgress(db, submissionId): Promise<BlockState[]>`
...
**Files:**
- Modify: `src/submissions/transitions.ts`
- Create: `src/review/decide.ts`
- Test: `src/review/__tests__/decide.test.ts`

**Interfaces:**
- Consumes: `Db`, `blockProgress`, `openFlags`, `FIELDS`, таблицы `submissions`, `lounges`, `events`
- Produces:
...
async function currentStatus(db: Db, submissionId: string): Promise<string | null> {
  const rows = await db
...
export async function requestChanges(
  ...
  const status = await currentStatus(db, input.submissionId)
  if (status !== 'submitted') {
...
export async function approveSubmission(
  ...
  const progress = await blockProgress(db, input.submissionId)
  ...
  const values = await loadSubmissionValues(db, input.submissionId)
  ...
  await db.update(lounges).set(classifying).where(eq(lounges.id, loungeId))
...
export async function requestChangesAction(
  ...
  if (!result.ok) return { ok: false, error: result.error.ru }
...
export async function resendFillLinkAction(
  ...
  const to = await contactEmail(submissionId)
  if (!to) {
    return { ok: false, error: 'у анкеты нет контактной почты (II.1.3)' }
  }
...
import { useState } from 'react'
import { BLOCKS, FIELDS, SERVICE_ITEMS, PHOTO_SLOTS } from '@/form-schema'
import type { BlockState } from '@/review/blocks'
...
**Files:**
- Modify: `src/review/decide.ts` — `resubmitAfterFixes`
- Modify: `src/app/f/[token]/page.tsx` — ветка на `changes_requested`
- Modify: `src/app/f/[token]/actions.ts` — согласование правки и снятия замечания
- Modify: `src/web/FixesOnly.tsx`
- Test: `src/review/__tests__/resubmit.test.ts`

**Interfaces:**
- Consumes: `openFlags`, `clearFlagsFor`, `saveFieldValue`
- Produces: `resubmitAfterFixes(db, { submissionId, actor }): Promise<TransitionResult>` в `src/review/decide.ts`

`resubmitAfterFixes` живёт в `review`, а не в `submissions`: он проверяет отсутствие открытых замечаний, а замечаниями владеет `review`. Обратная зависимость сломала бы границу модулей.
...
import { confirmBlock, blockProgress } from '../blocks'
import { resubmitAfterFixes } from '../decide'
...
describe('повторная отправка после правок', () => {
  it('пока замечание не исправлено, отправить нельзя', async () => {
    const db = await createTestDb()
    ...
    const result = await resubmitAfterFixes(db, { submissionId, actor: 'filler' })
    expect(result.ok).toBe(false)
  })

  it('правка отмеченного поля снимает замечание', async () => {
    const db = await createTestDb()
    const submissionId = await seedInFixes(db)
    await raiseFlag(db, {
      submissionId, fieldKey: 'I.2', reason: 'empty', comment: 'пусто', reviewer: 'r1',
  ...
  it('после правки всех замечаний анкета уходит на проверку', async () => {
    const db = await createTestDb()
    const submissionId = await seedInFixes(db)
    await raiseFlag(db, {
      submissionId, fieldKey: 'I.2', reason: 'empty', comment: 'пусто', reviewer: 'r1',
  ...
  it('подтверждённые ранее блоки остаются подтверждёнными', async () => {
    const db = await createTestDb()
    const submissionId = await seedInFixes(db)
    ...
    await saveFieldValue(db, { submissionId, fieldKey: 'I.2', value: 'Primeclass Lounge' })
    await clearFlagsFor(db, submissionId, 'I.2')
    await resubmitAfterFixes(db, { submissionId, actor: 'filler' })

    const progress = await blockProgress(db, submissionId)
    expect(progress.find((b) => b.blockKey === 'III.5')?.confirmed).toBe(true)
  })

  it('правка снимает подтверждение блока, к которому относилось замечание', async () => {
    const db = await createTestDb()
    const submissionId = await seedInFixes(db)
    await confirmBlock(db, { submissionId, blockKey: 'I', reviewer: 'r1' })
    await raiseFlag(db, {
  ...
  it('из draft повторная отправка не работает', async () => {
    const db = await createTestDb()
    const [lounge] = await db
      .insert(lounges)
      .values({ name: 'X', country: 'Turkey', city: 'Istanbul', airport: 'Istanbul Airport', iataCode: 'IST' })
      .returning()
    const [submission] = await db
      .insert(submissions).values({ loungeId: lounge!.id }).returning()

    const result = await resubmitAfterFixes(db, {
...
Expected: FAIL — `resubmitAfterFixes` не экспортируется.

- [ ] **Step 3: Согласовать правку и снятие замечания в слое действий**

`saveFieldValue` остаётся в неведении о замечаниях — иначе `submissions` начнёт
зависеть от `review`. Связывает их серверное действие, которое и так знает про
оба модуля. В `src/app/f/[token]/actions.ts` дописать импорт и вызов:

```ts
import { clearFlagsFor } from '@/review/flags'
```

и в `saveFieldAction` заменить хвост на:
...
- [ ] **Step 4: Дописать `resubmitAfterFixes` в `src/review/decide.ts`**

```ts
export async function resubmitAfterFixes(
  db: Db,
  input: { submissionId: string; actor: string },
): Promise<TransitionResult> {
  const status = await currentStatus(db, input.submissionId)
  if (status !== 'changes_requested') {
    return fail('Nothing to resubmit', 'Нечего отправлять повторно')
  }

  const flags = await openFlags(db, input.submissionId)
  if (flags.length > 0) {
    return fail(
      `${flags.length} flagged answer(s) still need a fix`,
      `Осталось исправить: ${flags.length}`,
    )
  }
...
`currentStatus`, `fail`, `openFlags`, `submissions` и `events` уже импортированы
в этот файл в Task 4 — новых импортов не требуется.


- [ ] **Step 5: Показать заполняющему только отмеченное**

В `src/app/f/[token]/page.tsx` после загрузки значений добавить:

```tsx
import { eq } from 'drizzle-orm'
import { submissions } from '@/db/schema'
import { openFlags } from '@/review/flags'
import { FixesOnly } from '@/web/FixesOnly'
import { FIELDS } from '@/form-schema'
```

и ветку перед возвратом `FillForm`:
...
```tsx
  ...
  if (statusRows[0]?.status === 'changes_requested') {
    const flags = await openFlags(db(), resolved.submissionId)
    return (
      <LocaleProvider initial="en">
        <FixesOnly
          token={token}
...
Дописать в `src/web/FixesOnly.tsx` собственное сохранение и кнопку повторной отправки: компонент становится клиентским владельцем своих значений.
...
import { saveFieldAction, resubmitAction } from '@/app/f/[token]/actions'
import type { FlagRow } from '@/review/flags'
...
export function FixesOnly(props: {
  ...
  return (
    <section className="fixes">
        ...
        return (
          <div key={flag.fieldKey} className="fix-card">
            ...
            {field && (
              <FieldInput
                field={field}
                value={values[flag.fieldKey]}
                onChange={(value) => {
                  setValues((prev) => ({ ...prev, [flag.fieldKey]: value }))
                  void saveFieldAction(props.token, flag.fieldKey, value)
                }}
              />
            )}
          </div>
      ...
      {error && <p className="fix-comment">{error}</p>}
      <button
        type="button"
        onClick={async () => {
          const result = await resubmitAction(props.token)
...
Дописать `resubmitAction` в `src/app/f/[token]/actions.ts`:

```ts
import { resubmitAfterFixes } from '@/review/decide'

export async function resubmitAction(token: string): Promise<ActionResult> {
...
```bash
git add src/review src/web src/app/f
git commit -m "feat(review): fixes-only view and resubmit after corrections"
```
...
**Interfaces:**
- Consumes: всё выше
- Produces: `npm run seed -- --complete` печатает ссылку заполнения и адрес почты команды

- [ ] **Step 1: Расширить сид**

Дописать в `scripts/seed-dev.ts` перед `process.stdout.write`:
  ...
  if (process.argv.includes('--complete')) {
    for (const field of FIELDS.filter((f) => f.required)) {
      const value =
        ...
        : 'заполнено'
      await saveFieldValue(db, { submissionId: submission!.id, fieldKey: field.key, value })
    }
    for (const item of SERVICE_ITEMS) {
    ...
    for (const slot of PHOTO_SLOTS.filter((s) => s.required)) {
      await db.insert(photos).values({
        submissionId: submission!.id, slot: slot.key,
        blobKey: `${slot.key}.jpg`, url: `https://example.test/${slot.key}.jpg`,
      })
    }
    await submitSubmission(db, { submissionId: submission!.id, actor: 'filler' })
  }
```
...
```ts
import { teamMembers, photos } from '../src/db/schema'
import { FIELDS, SERVICE_ITEMS, PHOTO_SLOTS, OPTION_LISTS } from '../src/form-schema'
...
и второй строкой вывода — идентификатор анкеты:
...
function seedComplete(): { fillUrl: string; submissionId: string } {
  const output = execSync('npm run --silent seed -- --complete', { encoding: 'utf8' })
  const [fillUrl, submissionId] = output.trim().split('\n')
  return { fillUrl: fillUrl!, submissionId: submissionId! }
}
...
test('ревьюер отмечает поле, возвращает анкету, заполняющий правит только его', async ({
  ...
  const { fillUrl, submissionId } = seedComplete()
  ...
  const loginUrl = execSync(
    `npx tsx scripts/dev-login-link.ts reviewer@easyto.travel`,
    { encoding: 'utf8' },
  ).trim()
  await page.goto(loginUrl)

  await page.goto(`/admin/s/${submissionId}`)
...
test('анкету нельзя принять, пока не подтверждены все блоки', async ({ page }) => {
  const { submissionId } = seedComplete()
