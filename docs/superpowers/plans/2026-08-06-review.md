...
**Interfaces:**
- Consumes: `Db`, `blockProgress`, `openFlags`, `extendFillToken`, `FIELDS`, таблицы `submissions`, `lounges`, `events`
- Produces:
...
import { extendFillToken } from '@/access/tokens'
import { openFlags } from './flags'
...
export async function requestChanges(
  ...
  // Ссылка заполняющего могла истечь, пока анкета лежала на проверке.
  await extendFillToken(db, input.submissionId, FIX_TOKEN_TTL_DAYS)
...
- Produces:
  ...
  - серверные действия `flagAction`, `unflagAction`, `confirmBlockAction`, `requestChangesAction`, `approveAction`
...
import {
  ...
  requestChangesAction, approveAction,
